...
public class GenerationRecorder implements AutoCloseable {
    ...
    GenerationRecorder(Agento11yClient client, GenerationStart seed, Span span, Instant startedAt,
    ...
    public void end() {
        ...
        try {
            ...
            Agento11yClient.stampContentCaptureMetadata(generation, contentCaptureMode);
            if (contentCaptureMode == ContentCaptureMode.METADATA_ONLY) {
                String errorCategory = Agento11yClient.errorCategoryFromThrowable(snapshotCallError, false);
                Agento11yClient.stripContent(generation, errorCategory);
            }
            ...
            span.updateName(Agento11yClient.generationSpanName(generation.getOperationName(), generation.getModel().getName()));
            ...
            if (contentCaptureMode == ContentCaptureMode.FULL_WITH_METADATA_SPANS) {
                ...
                try {
                    Agento11yClient.setGenerationSpanAttributes(span, generation);
                } finally {
            ...
            if (snapshotCallError != null) {
                ...
                span.setAttribute(Agento11yClient.SPAN_ATTR_ERROR_TYPE, "provider_call_error");
                ...
                span.setAttribute(Agento11yClient.SPAN_ATTR_ERROR_TYPE, "validation_error");
                span.setAttribute(Agento11yClient.SPAN_ATTR_ERROR_CATEGORY, errorCategory);
                ...
                errorCategory = "sdk_error";
                span.setAttribute(Agento11yClient.SPAN_ATTR_ERROR_TYPE, "enqueue_error");
                span.setAttribute(Agento11yClient.SPAN_ATTR_ERROR_CATEGORY, errorCategory);
                span.setStatus(StatusCode.ERROR, redactSpanErrors ? errorCategory : String.valueOf(localError.getMessage()));
            } else {
    ...
    private Generation normalize(
        ...
        generation.setId(firstNonBlank(result.getId(), seed.getId(), Agento11yClient.newID("gen")));
        ...
        if (operationName.isBlank()) {
            operationName = Agento11yClient.defaultOperationName(generation.getMode());
        }
        ...
        if (!result.getParentGenerationIds().isEmpty()) {
            generation.getParentGenerationIds().addAll(result.getParentGenerationIds());
        } else {
            generation.getParentGenerationIds().addAll(seed.getParentGenerationIds());
        }
        ...
        generation.setConversationTitle(firstNonBlank(
                ...
                Agento11yClient.metadataString(generation.getMetadata(), Agento11yClient.SPAN_ATTR_CONVERSATION_TITLE)));
        ...
        if (!generation.getConversationTitle().isBlank()) {
            generation.getMetadata().put(Agento11yClient.SPAN_ATTR_CONVERSATION_TITLE, generation.getConversationTitle());
        }

        generation.setUserId(firstNonBlank(
                generation.getUserId(),
                Agento11yClient.metadataString(generation.getMetadata(), Agento11yClient.METADATA_USER_ID_KEY),
                Agento11yClient.metadataString(generation.getMetadata(), Agento11yClient.METADATA_LEGACY_USER_ID_KEY)));
        generation.setUserId(normalizeResolvedString(generation.getUserId()));
        if (!generation.getUserId().isBlank()) {
            generation.getMetadata().put(Agento11yClient.METADATA_USER_ID_KEY, generation.getUserId());
        }
        ...
        generation.getMetadata().put(Agento11yClient.SPAN_ATTR_SDK_NAME, Agento11yClient.SDK_NAME);
