...
func mergeServerToolUsageMetadata(metadata map[string]any, usage asdk.BetaServerToolUsage) map[string]any {
	...
	if !serverToolUsageReported(usage) {
		return out
	}
	if out == nil {
		out = map[string]any{}
	}
	if tokenCountReported(usage.JSON.WebSearchRequests.Valid(), usage.WebSearchRequests) {
		out[usageServerToolUseWebSearchMetadataKey] = usage.WebSearchRequests
	}
	if tokenCountReported(usage.JSON.WebFetchRequests.Valid(), usage.WebFetchRequests) {
