...
func FromStream(req asdk.BetaMessageNewParams, summary StreamSummary, opts ...Option) (agento11y.Generation, error) {
	if summary.FinalMessage != nil {
		...
		usage, serverToolUsage := mapStreamUsage(summary.Events, summary.FinalMessage.Usage)
	...
	usage, serverToolUsage := mapStreamUsage(summary.Events, asdk.BetaUsage{})
	stopReason := ""
	modelName := req.Model
	responseID := ""
...
func mapStreamUsage(events []asdk.BetaRawMessageStreamEventUnion, initialUsage asdk.BetaUsage) (agento11y.TokenUsage, asdk.BetaServerToolUsage) {
	usage := mapUsage(initialUsage)
	rawInputTokens := initialUsage.InputTokens
	inputReported := usage.InputTokensReported
	outputReported := usage.OutputTokensReported
	serverToolUsage := initialUsage.ServerToolUse
	...
	for _, event := range events {
		...
		case "message_start":
			...
			serverToolUsage = mergeServerToolUsage(serverToolUsage, startUsage.ServerToolUse)
		case "message_delta":
			...
			serverToolUsage = mergeServerToolUsage(serverToolUsage, deltaUsage.ServerToolUse)
		}
	}
...
func mergeServerToolUsage(current, update asdk.BetaServerToolUsage) asdk.BetaServerToolUsage {
	if tokenCountReported(update.JSON.WebSearchRequests.Valid(), update.WebSearchRequests) {
		current.WebSearchRequests = update.WebSearchRequests
		current.JSON.WebSearchRequests = update.JSON.WebSearchRequests
	}
	if tokenCountReported(update.JSON.WebFetchRequests.Valid(), update.WebFetchRequests) {
		current.WebFetchRequests = update.WebFetchRequests
		current.JSON.WebFetchRequests = update.JSON.WebFetchRequests
	}
	return current
}
