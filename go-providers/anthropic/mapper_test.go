...
func TestFromStreamPreservesUsagePresenceAndCumulativeInput(t *testing.T) {
	...
	final := &asdk.BetaMessage{}
	for _, event := range events {
		if err := final.Accumulate(event); err != nil {
			t.Fatalf("accumulate event: %v", err)
		}
	}
	for _, finalMessage := range []*asdk.BetaMessage{nil, final} {
		generation, err := FromStream(testRequest(), StreamSummary{Events: events, FinalMessage: finalMessage})
		if err != nil {
			t.Fatalf("from stream: %v", err)
		}
	...
	cases := []struct {
		...
		finalUsage       string
		eventUsage       string
		wantCounts       [5]int64 // inclusive input, output, cache read, cache write, total
		wantAbsentInput  bool
		wantAbsentOutput bool
		wantTools        *[2]int64 // web search, web fetch
	}{
		{name: "output-only regression", finalUsage: `{"input_tokens":10,"output_tokens":4}`, eventUsage: `{"output_tokens":4}`, wantCounts: [5]int64{10, 4, 0, 0, 14}},
		{name: "no events", wantCounts: [5]int64{18, 4, 6, 2, 22}},
		{name: "absent event counters", eventUsage: `{}`, wantCounts: [5]int64{18, 4, 6, 2, 22}},
		{name: "output preserves cache buckets", eventUsage: `{"output_tokens":5}`, wantCounts: [5]int64{18, 5, 6, 2, 23}},
		{name: "raw input preserves cache buckets", eventUsage: `{"input_tokens":20}`, wantCounts: [5]int64{28, 4, 6, 2, 32}},
		{name: "cache read preserves raw input", eventUsage: `{"cache_read_input_tokens":9}`, wantCounts: [5]int64{21, 4, 9, 2, 25}},
		{name: "cache write preserves raw input", eventUsage: `{"cache_creation_input_tokens":7}`, wantCounts: [5]int64{23, 4, 6, 7, 27}},
		{name: "zero raw input", eventUsage: `{"input_tokens":0}`, wantCounts: [5]int64{8, 4, 6, 2, 12}},
		{name: "zero cache read", eventUsage: `{"cache_read_input_tokens":0}`, wantCounts: [5]int64{12, 4, 0, 2, 16}},
		{name: "zero cache write", eventUsage: `{"cache_creation_input_tokens":0}`, wantCounts: [5]int64{16, 4, 6, 0, 20}},
		{name: "zero output", eventUsage: `{"output_tokens":0}`, wantCounts: [5]int64{18, 0, 6, 2, 18}},
		{name: "all reported zeros", eventUsage: `{"input_tokens":0,"cache_read_input_tokens":0,"cache_creation_input_tokens":0,"output_tokens":0}`},
		{name: "final reported zero input", finalUsage: `{"input_tokens":0}`, eventUsage: `{"output_tokens":4}`, wantCounts: [5]int64{0, 4, 0, 0, 4}},
		{name: "final reported zero output", finalUsage: `{"output_tokens":0}`, eventUsage: `{"input_tokens":10}`, wantCounts: [5]int64{10, 0, 0, 0, 10}},
		{name: "absent input has no total", finalUsage: `{}`, eventUsage: `{"output_tokens":4}`, wantCounts: [5]int64{0, 4, 0, 0, 0}, wantAbsentInput: true},
		{name: "absent output has no total", finalUsage: `{}`, eventUsage: `{"input_tokens":10}`, wantCounts: [5]int64{10, 0, 0, 0, 0}, wantAbsentOutput: true},
		{name: "server tool only", eventUsage: `{"server_tool_use":{"web_search_requests":7}}`, wantCounts: [5]int64{18, 4, 6, 2, 22}, wantTools: &[2]int64{7, 2}},
		{name: "zero server tool with tokens", eventUsage: `{"output_tokens":5,"server_tool_use":{"web_fetch_requests":0}}`, wantCounts: [5]int64{18, 5, 6, 2, 23}, wantTools: &[2]int64{3, 0}},
		{name: "all server tools zero", eventUsage: `{"server_tool_use":{"web_search_requests":0,"web_fetch_requests":0}}`, wantCounts: [5]int64{18, 4, 6, 2, 22}, wantTools: &[2]int64{}},
	}
	for _, tc := range cases {
		for _, eventType := range []string{"message_start", "message_delta"} {
			t.Run(tc.name+"/"+eventType, func(t *testing.T) {
				final := &asdk.BetaMessage{Model: "claude-sonnet-4-5"}
				finalUsage := tc.finalUsage
				if finalUsage == "" {
					finalUsage = `{"input_tokens":10,"output_tokens":4,"cache_read_input_tokens":6,"cache_creation_input_tokens":2}`
				}
				if err := json.Unmarshal([]byte(finalUsage), &final.Usage); err != nil {
					t.Fatalf("decode final usage: %v", err)
				}
				final.Usage.ServerToolUse = asdk.BetaServerToolUsage{WebSearchRequests: 3, WebFetchRequests: 2}
				summary := StreamSummary{FinalMessage: final}
				if tc.eventUsage != "" {
					payload := `{"type":"message_delta","usage":` + tc.eventUsage + `}`
					if eventType == "message_start" {
						payload = `{"type":"message_start","message":{"usage":` + tc.eventUsage + `}}`
					}
					var event asdk.BetaRawMessageStreamEventUnion
					if err := json.Unmarshal([]byte(payload), &event); err != nil {
						t.Fatalf("decode sparse event: %v", err)
					}
					summary.Events = []asdk.BetaRawMessageStreamEventUnion{event}
				}
				generation, err := FromStream(testRequest(), summary)
				if err != nil {
					t.Fatalf("map sparse stream: %v", err)
				}
				usage := generation.Usage
				counts := [5]int64{usage.InputTokens, usage.OutputTokens, usage.CacheReadInputTokens, usage.CacheWriteInputTokens, usage.TotalTokens}
				if counts != tc.wantCounts {
					t.Errorf("usage counts = %v, want %v", counts, tc.wantCounts)
				}
				if usage.InputTokensReported != !tc.wantAbsentInput || usage.OutputTokensReported != !tc.wantAbsentOutput {
					t.Errorf("unexpected usage presence: %+v", usage)
				}
				if usage.InputSemantics != agento11y.TokenInputSemanticsInclusive {
					t.Errorf("input semantics = %v, want inclusive", usage.InputSemantics)
				}
				wantTools := [2]int64{3, 2}
				if tc.wantTools != nil {
					wantTools = *tc.wantTools
				}
				for key, want := range map[string]int64{
					usageServerToolUseWebSearchMetadataKey: wantTools[0],
					usageServerToolUseWebFetchMetadataKey:  wantTools[1],
					usageServerToolUseTotalMetadataKey:     wantTools[0] + wantTools[1],
				} {
					if got := generation.Metadata[key]; got != want {
						t.Errorf("metadata %s = %v, want %d", key, got, want)
					}
				}
			})
		}
