...
func TestOTelResponseErrorMapping(t *testing.T) {
	...
	for _, api := range []string{"chat", "responses"} {
		for _, tc := range []struct {
			...
			{name: "nil response nil error", message: "response is required"},
			{name: "nil response provider error", err: providerErr, message: providerErr.Error()},
			{name: "partial response provider error", err: providerErr, partial: true, message: providerErr.Error()},
		} {
			t.Run(api+"/"+tc.name, func(t *testing.T) {
				env := testkit.NewOTelEnv(t)
				switch api {
				case "chat":
					var native *osdk.ChatCompletion
					if tc.partial {
						native = &osdk.ChatCompletion{ID: "partial", Choices: []osdk.ChatCompletionChoice{{Message: osdk.ChatCompletionMessage{Content: "partial answer"}}}}
					}
					response, err := chatCompletionsNew(context.Background(), env.Client,
						osdk.ChatCompletionNewParams{Model: "gpt-5"},
						func(context.Context, osdk.ChatCompletionNewParams) (*osdk.ChatCompletion, error) {
							return native, tc.err
						},
					)
					if response != native || !errors.Is(err, tc.err) {
						t.Fatalf("native return = (%p, %v), want (%p, %v)", response, err, native, tc.err)
					}
				case "responses":
					var native *responses.Response
					if tc.partial {
						native = &responses.Response{ID: "partial", Output: []responses.ResponseOutputItemUnion{{Type: "message", Content: []responses.ResponseOutputMessageContentUnion{{Type: "output_text", Text: "partial answer"}}}}}
					}
					response, err := responsesNew(context.Background(), env.Client,
						responses.ResponseNewParams{Model: "gpt-5"},
						func(context.Context, responses.ResponseNewParams) (*responses.Response, error) { return native, tc.err },
					)
					if response != native || !errors.Is(err, tc.err) {
						t.Fatalf("native return = (%p, %v), want (%p, %v)", response, err, native, tc.err)
					}
				...
				if span.Status().Code != codes.Error || !strings.Contains(span.Status().Description, tc.message) {
					t.Fatalf("span status = (%v, %q), want %q", span.Status().Code, span.Status().Description, tc.message)
				}
				requireOTelException(t, span, tc.message)
				attrs := testkit.SpanAttributes(span)
				if got := attrs["error.type"].AsString(); got == "" {
					t.Fatal("error span has no error.type")
				}
				if tc.partial {
					requireOTelStringAttr(t, attrs, "gen_ai.response.id", "partial")
...
func requireOTelException(t *testing.T, span sdktrace.ReadOnlySpan, message string) {
	t.Helper()
	var exceptions []sdktrace.Event
	...
	if len(exceptions) != 1 {
		t.Fatalf("exception count = %d, want 1: %#v", len(exceptions), exceptions)
	}
	for _, attr := range exceptions[0].Attributes {
		if attr.Key == "exception.message" && strings.Contains(attr.Value.AsString(), message) {
			return
		}
	}
	t.Fatalf("exception = %#v, want message %q", exceptions[0], message)
}
