...
func TestRequestControlsPreserveSeed(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "absent"},
		{name: "zero", seed: param.NewOpt(int64(0)), want: int64Ptr(0)},
		{name: "positive", seed: param.NewOpt(int64(9007199254740993)), want: int64Ptr(9007199254740993)},
		{name: "negative", seed: param.NewOpt(int64(-9007199254740993)), want: int64Ptr(-9007199254740993)},
		{name: "maximum", seed: param.NewOpt(int64(math.MaxInt64)), want: int64Ptr(math.MaxInt64)},
		{name: "minimum", seed: param.NewOpt(int64(math.MinInt64)), want: int64Ptr(math.MinInt64)},
		{name: "override", seed: param.NewOpt(int64(17)), extra: map[string]any{"seed": int64(9007199254740993)}, want: int64Ptr(9007199254740993)},
		{name: "override zero", seed: param.NewOpt(int64(17)), extra: map[string]any{"seed": int64(0)}, want: int64Ptr(0)},
		{name: "override null", seed: param.NewOpt(int64(17)), extra: map[string]any{"seed": nil}},
	} {
		for _, api := range []string{"chat", "responses"} {
			t.Run(api+"/"+tc.name, func(t *testing.T) {
				var request any
				var generation agento11y.Generation
				var err error
				if api == "chat" {
					req := osdk.ChatCompletionNewParams{
						Model: "gpt-5", Seed: tc.seed, N: param.NewOpt(int64(2)),
						MaxCompletionTokens: param.NewOpt(int64(128)),
						Temperature:         param.NewOpt(0.7), TopP: param.NewOpt(0.9),
					}
					req.SetExtraFields(tc.extra)
					request = req
					generation, err = ChatCompletionsFromRequestResponse(req, &osdk.ChatCompletion{}, WithRawArtifacts())
				} else {
					req := oresponses.ResponseNewParams{
						Model: "gpt-5", MaxOutputTokens: param.NewOpt(int64(128)),
						Temperature: param.NewOpt(0.7), TopP: param.NewOpt(0.9),
					}
					extra := map[string]any{"n": 2}
					if tc.seed.Valid() {
						extra["seed"] = tc.seed.Value
					}
					maps.Copy(extra, tc.extra)
					req.SetExtraFields(extra)
					request = req
					generation, err = ResponsesFromRequestResponse(req, &oresponses.Response{Status: oresponses.ResponseStatusCompleted}, WithRawArtifacts())
				}
				if err != nil {
				...
				if tc.want == nil {
					if generation.Seed != nil {
						t.Errorf("seed = %d, want absent", *generation.Seed)
					}
				} else if generation.Seed == nil {
					t.Errorf("seed absent, want %d", *tc.want)
				} else if *generation.Seed != *tc.want {
					t.Errorf("seed = %d, want %d", *generation.Seed, *tc.want)
				}
				if generation.Temperature == nil || *generation.Temperature != 0.7 || generation.TopP == nil || *generation.TopP != 0.9 {
					t.Errorf("temperature/top_p = %v/%v, want 0.7/0.9", generation.Temperature, generation.TopP)
				}
				if generation.MaxTokens == nil || *generation.MaxTokens != 128 || generation.ChoiceCount == nil || *generation.ChoiceCount != 2 {
					t.Errorf("max tokens/choice count = %v/%v, want 128/2", generation.MaxTokens, generation.ChoiceCount)
				}
				...
				if len(generation.Artifacts) == 0 {
					t.Fatal("request artifact absent")
				}
				var gotArtifact, wantArtifact map[string]json.RawMessage
				if err := json.Unmarshal(generation.Artifacts[0].Payload, &gotArtifact); err != nil {
					t.Fatal(err)
				}
				if err := json.Unmarshal(wantJSON, &wantArtifact); err != nil {
					t.Fatal(err)
				}
				if !reflect.DeepEqual(gotArtifact, wantArtifact) {
					t.Fatalf("request artifact = %s, want %s", generation.Artifacts[0].Payload, wantJSON)
				}
			})
		}
	}
