...
// otelFlusher is the part of a TracerProvider that Flush needs. The OTel SDK's
// provider implements this interface; a caller's wrapper might not.
type otelFlusher interface {
	ForceFlush(ctx context.Context) error
}

// flushOTel force-flushes the tracer provider, which is where a generation
// goes in otel mode. Without a provider the SDK holds a tracer and nothing
// else. A nil return would then tell the caller that a batch is delivered
// while the span processor might still hold that batch.
func (c *Client) flushOTel(ctx context.Context) error {
	flusher, ok := c.config.TracerProvider.(otelFlusher)
	if !ok {
		return fmt.Errorf("%w: set Config.TracerProvider to a provider with ForceFlush", ErrFlushNotVerifiable)
	}
...
//
// The generation-export handler gets its tracer and spec meter from
// Config.TracerProvider and Config.MeterProvider, or the corresponding global
// providers. Config.Tracer and Config.Meter do not affect generation export;
// they continue to configure the SDK's tool-execution and embedding telemetry
// in every mode.
//
...
func otelHandlerOptions(cfg Config) []otelgenai.Option {
	return []otelgenai.Option{
		otelgenai.WithCompletionHook(otelhook.New()),
...
// low-cardinality error.type, the SDK's error category, and the text that
// rides on the span status. All three are empty on success.
...
func (c *Client) endOTelGeneration(
	...
	// The full context goes in. Unlike the proprietary path, which records its
	// metrics here and has to strip the context itself, otelgenai keeps only
	// the span identity before it records.
	c.otelHandler.End(ctx, invocation)
}
...
func applyGenerationToInvocation(
	...
	// instant, and nothing can move that instant. The metrics measure from the
	// same instant, so a generation's own start would disagree with both. A
	// mapper that reports a provider-side start therefore changes the duration
	// on the proprietary path and leaves the duration on this path alone.
	...
	invocation.Vendor = otelhook.Generation{
		...
		// The proprietary path exports the digest of the effective version, so
		// the same agent build has to hash to the same value on both paths.
		EffectiveVersion:        codec.EffectiveVersionDigest(generation.EffectiveVersion),
...
// and the metric dimension carry. Any other name is a caller's own and rides
// through unchanged.
...
// mislabeled payload and rides as a uri instead. The comparison ignores case,
// and an undeclared mime type takes the data URL's mime type, so neither of
// those two costs the blob shape the schema reserves for inline content.
...
// `response` key. Structured results ride raw; anything else rides as a JSON
// string with the exact bytes on the base64 extension key, which is the shape
// the backend decoder inverts.
...
// otelEmbeddableJSON reports whether raw can ride as a raw JSON document and
// decode back to the same value. The bytes must already be compact, because
