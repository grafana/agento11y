...
func TestOnCompletion(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			want: map[string]string{
				...
				"agento11y.agent.effective_version":          "v3",
		...
		{
			name:    "content capture off drops the content mirrors and artifacts",
			...
			absent: []string{"agento11y.generation.raw_artifacts"},
		},
		{
			name:    "content capture on keeps the title and the artifacts",
			capture: otelgenai.CaptureSpanOnly,
			vendor: otelhook.Generation{
				ID:                "gen-4",
				ConversationTitle: "Fix the flake",
				// A sanitizer that rebuilt the metadata map drops the mirror,
				// and the title would otherwise reach the backend nowhere.
				Metadata:  nil,
				Artifacts: []otelhook.Artifact{{Kind: "request", ContentType: "application/json", Payload: []byte(`{"a":1}`)}},
			},
			want: map[string]string{
				"agento11y.generation.metadata":      `{"agento11y.conversation.title":"Fix the flake"}`,
				"agento11y.generation.raw_artifacts": `[{"kind":"request","content_type":"application/json","payload_b64":"eyJhIjoxfQ=="}]`,
			},
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			got := attributeMap(otelhook.New().OnCompletion(&otelgenai.Invocation{Vendor: tc.vendor}, tc.capture))
...
func TestOnCompletionNilInvocation(t *testing.T) {
	t.Parallel()

	if got := otelhook.New().OnCompletion(nil, otelgenai.CaptureSpanOnly); got != nil {
		t.Fatalf("OnCompletion(nil) = %v, want nil", got)
	}
}

// TestOnCompletionReportsDroppedPayloads pins that a payload the hook cannot
// encode reaches the OTel error handler. Dropping the payload silently leaves
// a span that looks healthy while the backend cannot decode that span as a
// generation. The proprietary path fails the export outright on the same input.
//
// The error handler is global, so these cases cannot run in parallel.
func TestOnCompletionReportsDroppedPayloads(t *testing.T) {
	cases := []struct {
		name   string
		vendor any
		want   string
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			otelhook.New().OnCompletion(&otelgenai.Invocation{Vendor: tc.vendor}, otelgenai.CaptureSpanOnly)
...
// TestOnCompletionStaysQuietForForeignInvocations pins the other half of that
// contract: the hook can share a handler with instrumentation that has no
// vendor payload at all, and reporting those invocations would put one line
// per span in the host application's error handler.
func TestOnCompletionStaysQuietForForeignInvocations(t *testing.T) {
	var reported []string
	previous := otel.GetErrorHandler()
	otel.SetErrorHandler(otel.ErrorHandlerFunc(func(err error) {
		reported = append(reported, err.Error())
	...
	otelhook.New().OnCompletion(&otelgenai.Invocation{}, otelgenai.CaptureSpanOnly)
