...
func TestConformance_FullGenerationRoundtrip(t *testing.T) {
	...
	requestArtifact, err := agento11y.NewJSONArtifact(agento11y.ArtifactKindRequest, "request", map[string]any{
		"messages": 1,
		"tools":    1,
	})
	if err != nil {
	...
	responseArtifact, err := agento11y.NewJSONArtifact(agento11y.ArtifactKindResponse, "response", map[string]any{
		"response_id": "msg_1",
		"status":      "ok",
	})
	if err != nil {
	...
	recorder.SetResult(agento11y.Generation{
		...
		Input: []agento11y.Message{
			{
				...
				Parts: []agento11y.Part{
					agento11y.TextPart("Summarize yesterday's Paris weather and explain the spikes."),
				},
			},
		},
		Output: []agento11y.Message{
				...
				Parts: []agento11y.Part{
					{
						...
						Metadata: agento11y.PartMetadata{ProviderType: "thinking"},
					},
					{
						...
						Metadata: agento11y.PartMetadata{ProviderType: "tool_use"},
					},
				},
			},
			{
			...
			},
			{
				...
				Name: "assistant",
				Parts: []agento11y.Part{
					agento11y.TextPart("Paris peaked at 22C before a late drop as cloud cover moved in."),
				},
			},
		},
		...
		Artifacts: []agento11y.Artifact{requestArtifact, responseArtifact},
	}, nil)
	recorder.End()
...
func TestConformance_WorkflowStepRoundtrip(t *testing.T) {
	...
	if err := env.Client.EnqueueWorkflowStep(agento11y.WorkflowStep{
		ID:                  "wfs-roundtrip",
...
func TestConformance_ConversationTitleSemantics(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.contextTitle != "" {
				ctx = agento11y.WithConversationTitle(ctx, tc.contextTitle)
			}
			...
			recordGeneration(t, ctx, env, start, agento11y.Generation{})
...
func TestConformance_UserIDSemantics(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			ctx := context.Background()
			if tc.contextUserID != "" {
				ctx = agento11y.WithUserID(ctx, tc.contextUserID)
			}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.contextAgentName != "" {
				ctx = agento11y.WithAgentName(ctx, tc.contextAgentName)
			}
			if tc.contextVersion != "" {
				ctx = agento11y.WithAgentVersion(ctx, tc.contextVersion)
			}
...
func TestConformance_EffectiveVersionSemantics(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			recordGeneration(t, context.Background(), env, start, agento11y.Generation{})
...
func TestConformance_EffectiveVersionResultOverridesStart(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			result := agento11y.Generation{EffectiveVersion: tc.resultValue}
...
func TestConformance_StreamingMode(t *testing.T) {
	...
	recordGeneration(t, context.Background(), env, agento11y.GenerationStart{
	...
	_, recorder := env.Client.StartStreamingGeneration(context.Background(), agento11y.GenerationStart{
	...
	recorder.SetResult(agento11y.Generation{
		...
		Output: []agento11y.Message{agento11y.AssistantTextMessage("Hello world")},
		Usage: agento11y.TokenUsage{
			InputTokens:  3,
			OutputTokens: 2,
			TotalTokens:  5,
...
func TestConformance_ToolExecution(t *testing.T) {
	...
	ctx := agento11y.WithConversationID(context.Background(), "conv-tool")
	ctx = agento11y.WithConversationTitle(ctx, "Weather lookup")
	ctx = agento11y.WithAgentName(ctx, "agent-tools")
	ctx = agento11y.WithAgentVersion(ctx, "2026.03.12")

	generationStartedAt := time.Date(2026, 3, 12, 14, 2, 0, 0, time.UTC)
	callCtx, generationRecorder := env.Client.StartGeneration(ctx, agento11y.GenerationStart{
		Model:     conformanceModel,
		StartedAt: generationStartedAt,
	})
	_, toolRecorder := env.Client.StartToolExecution(callCtx, agento11y.ToolExecutionStart{
		ToolName:        "weather",
	...
	toolRecorder.SetResult(agento11y.ToolExecutionEnd{
	...
	generationRecorder.SetResult(agento11y.Generation{
		Input:       []agento11y.Message{agento11y.UserTextMessage("weather in Paris")},
		Output:      []agento11y.Message{agento11y.AssistantTextMessage("Paris is 18C")},
...
func TestConformance_Embedding(t *testing.T) {
	...
	recorder.SetResult(agento11y.EmbeddingResult{
		InputCount:    2,
		InputTokens:   120,
...
func TestConformance_FullWithMetadataSpansMode(t *testing.T) {
	t.Run("generation proto full, span title absent", func(t *testing.T) {
		env := newConformanceEnv(t, withConformanceConfig(func(cfg *agento11y.Config) {
			cfg.ContentCapture = agento11y.ContentCaptureModeFullWithMetadataSpans
		}))

		requestArtifact, err := agento11y.NewJSONArtifact(agento11y.ArtifactKindRequest, "request", map[string]any{"messages": 1})
		if err != nil {
			t.Fatalf("build request artifact: %v", err)
		}
		...
		recorder.SetResult(agento11y.Generation{
			Input:  []agento11y.Message{agento11y.UserTextMessage("hello world")},
			Output: []agento11y.Message{agento11y.AssistantTextMessage("hi back")},
			Usage: agento11y.TokenUsage{
				InputTokens:  3,
				OutputTokens: 2,
				TotalTokens:  5,
			},
			Artifacts:   []agento11y.Artifact{requestArtifact},
	...
	t.Run("tool execution proto full, span content absent", func(t *testing.T) {
		...
		ctx := agento11y.WithConversationID(context.Background(), "conv-fwms-tool")
		ctx = agento11y.WithConversationTitle(ctx, "Tool sensitive title")
		...
		generationRecorder.SetResult(agento11y.Generation{
			Input: []agento11y.Message{agento11y.UserTextMessage("weather please")},
			Output: []agento11y.Message{
				{
					Role: agento11y.RoleAssistant,
					Parts: []agento11y.Part{
	...
	t.Run("tool execution call error redacted on span", func(t *testing.T) {
		...
		_, toolRec := env.Client.StartToolExecution(context.Background(), agento11y.ToolExecutionStart{
			ToolName:        "weather",
		...
		toolRec.SetResult(agento11y.ToolExecutionEnd{
			Arguments:   map[string]any{"city": "Paris"},
			Result:      map[string]any{"temp_c": 18},
	...
	t.Run("embedding provider call error redacted on span", func(t *testing.T) {
		...
		env := newConformanceEnv(t, withConformanceConfig(func(cfg *agento11y.Config) {
			cfg.ContentCapture = agento11y.ContentCaptureModeFullWithMetadataSpans
			cfg.EmbeddingCapture = agento11y.EmbeddingCaptureConfig{CaptureInput: true}
		}))
	...
	t.Run("rating comment preserved", func(t *testing.T) {
		...
		_, err := env.Client.SubmitConversationRating(context.Background(), "conv-fwms-rating", agento11y.ConversationRatingInput{
			RatingID: "rat-fwms",
			Rating:   agento11y.ConversationRatingValueGood,
		...
		var payload agento11y.ConversationRatingInput
	...
	t.Run("rating comment stripped under MetadataOnly", func(t *testing.T) {
		...
		_, err := env.Client.SubmitConversationRating(context.Background(), "conv-meta-rating", agento11y.ConversationRatingInput{
			RatingID: "rat-meta",
			Rating:   agento11y.ConversationRatingValueGood,
			Comment:  "user-supplied free text",
		})
	...
	t.Run("provider call error redacted on span, raw in proto", func(t *testing.T) {
		...
		recorder.SetCallError(errors.New(rawErr))
		recorder.SetResult(agento11y.Generation{
			Input:       []agento11y.Message{agento11y.UserTextMessage("x")},
			Output:      []agento11y.Message{agento11y.AssistantTextMessage("y")},
...
func TestConformance_ValidationAndErrorSemantics(t *testing.T) {
	t.Run("invalid generation", func(t *testing.T) {
		...
		if err := recorder.Err(); !errors.Is(err, agento11y.ErrValidationFailed) {
	...
	t.Run("provider call error", func(t *testing.T) {
		...
		recorder.SetResult(agento11y.Generation{
			Input:       []agento11y.Message{agento11y.UserTextMessage("retry later")},
			Output:      []agento11y.Message{agento11y.AssistantTextMessage("rate limited")},
...
func TestConformance_RatingHelper(t *testing.T) {
	...
	response, err := env.Client.SubmitConversationRating(context.Background(), "conv-rated", agento11y.ConversationRatingInput{
		RatingID: "rat-1",
		Rating:   agento11y.ConversationRatingValueGood,
		Comment:  "looks good",
	...
	if payload.Rating != agento11y.ConversationRatingValueGood {
...
func recordGeneration(t *testing.T, ctx context.Context, env *conformanceEnv, start agento11y.GenerationStart, result agento11y.Generation) {
	t.Helper()
...
var contentCaptureModeMatrix = []modeExpect{
	...
	},
	{
		...
		mode:                 agento11y.ContentCaptureModeNoToolContent,
	...
	},
	{
		...
		mode:                 agento11y.ContentCaptureModeFullWithMetadataSpans,
...
// across all modes.
func fullContentGeneration() agento11y.Generation {
	return agento11y.Generation{
		SystemPrompt: "You are helpful.",
		Input: []agento11y.Message{
			agento11y.UserTextMessage("What is the weather?"),
			{
				Role: agento11y.RoleTool,
				Parts: []agento11y.Part{
					{
		...
		Output: []agento11y.Message{
			{
				Role: agento11y.RoleAssistant,
				Parts: []agento11y.Part{
					{Kind: agento11y.PartKindThinking, Thinking: "let me think about weather"},
					{
						Kind: agento11y.PartKindToolCall,
						ToolCall: &agento11y.ToolCall{
							ID:        "call_1",
					...
					{Kind: agento11y.PartKindText, Text: "It's 18C and sunny in Paris."},
				},
			},
		},
		Tools: []agento11y.ToolDefinition{
...
func TestConformance_ContentCaptureModeMatrix(t *testing.T) {
	...
	for _, tc := range contentCaptureModeMatrix {
		t.Run(tc.name+"/generation", func(t *testing.T) {
			...
			_, rec := env.Client.StartGeneration(context.Background(), agento11y.GenerationStart{
	...
	t.Run("streaming_full_with_metadata_spans", func(t *testing.T) {
		...
		_, rec := env.Client.StartStreamingGeneration(context.Background(), agento11y.GenerationStart{
		...
		rec.SetResult(agento11y.Generation{
			...
			Input:        []agento11y.Message{agento11y.UserTextMessage("hello")},
			Output:       []agento11y.Message{agento11y.AssistantTextMessage("hi")},
			Usage:        agento11y.TokenUsage{InputTokens: 1, OutputTokens: 1, TotalTokens: 2},
...
func TestConformance_NoSDKOwnedLegacySigilNamespace(t *testing.T) {
	...
	recorder.SetResult(agento11y.Generation{
		Output: []agento11y.Message{
			{Role: agento11y.RoleAssistant, Parts: []agento11y.Part{agento11y.TextPart("ok")}},
		},
		Usage: agento11y.TokenUsage{InputTokens: 1, OutputTokens: 1, TotalTokens: 2},
	}, nil)
	recorder.End()
