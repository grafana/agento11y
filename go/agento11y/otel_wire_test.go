...
// The fixtures under testdata/otlpwire are copied from the backend receiver
// (sigil/internal/ingest/otlptrace/testdata) and pin the span wire format on
// both sides. The backend asserts that GenerationFromSpan turns each span
// fixture back into its generation fixture; this test asserts the SDK emits
// that span for that generation in otel mode.
//
// Two things deliberately differ from the fixture, because otel mode is
// semconv-first whereas the backend encoder replays whatever the record holds:
//
//   - the operation name is always "chat", never the SDK's generateText /
//     streamText, and the span name follows it;
//   - attribute order is the util's, not the encoder's, so attributes are
//     compared as a set. Order carries no meaning on the wire.
//
// The generation goes through the client's otel-mode helpers rather than a
// recorder, because openai_sync holds a system-role message that the SDK's
// own validation rejects. The fixtures are the backend's data model, which is
// wider than what the SDK can produce.
//
// When the wire format changes, regenerate in the backend repo and copy the
// fixtures here.
