...
func (i instruments) recordTimeToFirstChunk(ctx context.Context, inv *Invocation, seconds float64) {
	i.timeToFirstChunk.RecordSet(metricExemplarContext(ctx), seconds, metricAttributeSet(inv, metricAttributes(inv)...))
}

func (i instruments) recordTimePerOutputChunk(ctx context.Context, inv *Invocation, seconds float64) {
	i.timePerOutputChunk.RecordSet(metricExemplarContext(ctx), seconds, metricAttributeSet(inv, metricAttributes(inv)...))
}

// metricExemplarContext keeps the span identity an exemplar needs and drops
// everything else the caller's context holds. An exemplar reservoir retains
// the context it recorded with until that slot is replaced, which would hold
// the recording span from Start, its attributes, and any request values alive
// for the life of the series. A span context is immutable and small.
