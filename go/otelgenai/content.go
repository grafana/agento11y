...
// SPAN_AND_EVENT. The package treats anything else as NO_CONTENT.
...
//
// A completion hook reads the mode to decide whether an attribute the hook
// adds may carry content. The handler cannot make that call, because it
// cannot see inside a hook's attributes.
...
func CaptureModeFromEnv() CaptureMode {
	mode, _ := ParseCaptureMode(os.Getenv(EnvCaptureMessageContent))
	return mode
}
