...
_expected_for_span(span, op) := expected if {
	span.status.code != "error"
	# A successful fetch can return before generation has finished.
	unavailable := {attr |
		op == "fetch_response"
		_attr_value(span, "gen_ai.response.status") in {"queued", "in_progress"}
		attr := "gen_ai.response.finish_reasons"
	}
	expected := _expected_for_op(op, span.kind) - unavailable
}
