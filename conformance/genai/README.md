...
The `test:go:conformance` task in `mise.toml` pins Weaver and the `open-telemetry/semantic-conventions-genai` commit. Mise installs and runs the pinned Weaver release for local and CI runs.
...
Run the suite through mise so it installs the pinned Weaver release and sets the registry commit:

```sh
mise run test:go:conformance
```

The task disables Go's test cache because the suite reads the registry and policy files at runtime. Each scenario rejects violation-level findings unless its allowlist names the advice ID and part of the message. The agento11y scenario allows only `missing_attribute` findings for its `agento11y.*` extension attributes.
