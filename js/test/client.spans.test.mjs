...
test('generation span marks disjoint token semantics only for normalized usage', async () => {
  const harness = newHarness();
  ...
  try {
    ...
    markedRecorder.setResult({
      usage: { inputTokens: 10, outputTokens: 4, cacheReadInputTokens: 3, inputIsDisjoint: true },
    });
    markedRecorder.end();
    assert.equal(markedRecorder.getError(), undefined);

    const markedSpan = singleGenerationSpan(harness.spanExporter);
    assert.equal(markedSpan.attributes['gen_ai.token.semantics'], 'disjoint');
