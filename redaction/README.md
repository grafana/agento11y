...
The inactive [Hermes snapshot migration](../plugins/hermes/README.md) reuses Python SDK 0.17.x redaction. It adds no engine or generated pattern table. The historical PyPI `0.10.0` only bounded payload structure; the migration adds shared secret redaction and changes capture to metadata-only by default. Hermes sanitizes tool-execution spans separately because they do not pass through the generation sanitizer.
...
The table says what a pattern matches; the caller decides which tier runs. The SDKs' generation sanitizer and the coding-agent plugins, including the Hermes migration, split it the same way: tier 1 + tier 2 on a user prompt, a system prompt and a tool payload,
...
Pi and the Hermes migration use their SDK sanitizers and redact email addresses.
