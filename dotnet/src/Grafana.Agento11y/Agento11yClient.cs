...
public sealed partial class Agento11yClient : IAsyncDisposable
{
    ...
    internal const string SpanAttrCacheWrite1hTokens = "gen_ai.usage.cache_write1h_input_tokens";
    ...
    internal const string MetricTokenTypeCacheWrite1h = "cache_write1h";
    ...
    internal readonly Agento11yClientConfig _config;
    ...
    public Agento11yClient(Agento11yClientConfig? config = null)
    {
    ...
    public EmbeddingRecorder StartEmbedding(EmbeddingStart start)
    {
        ...
        {
            seed.AgentName = Agento11yContext.AgentNameFromContext() ?? string.Empty;
        }
        if (string.IsNullOrWhiteSpace(seed.AgentName))
        {
        ...
        {
            seed.AgentVersion = Agento11yContext.AgentVersionFromContext() ?? string.Empty;
        }
        if (string.IsNullOrWhiteSpace(seed.AgentVersion))
        {
    ...
    public ToolExecutionRecorder StartToolExecution(ToolExecutionStart start)
    {
        ...
        {
            seed.ConversationId = Agento11yContext.ConversationIdFromContext() ?? string.Empty;
        }

        if (string.IsNullOrWhiteSpace(seed.ConversationTitle))
        {
            seed.ConversationTitle = Agento11yContext.ConversationTitleFromContext() ?? string.Empty;
        }
        ...
        var ctxMode = Agento11yContext.ContentCaptureModeFromContext();
        var ctxSet = Agento11yContext.HasContentCaptureModeInContext();
...
#pragma warning restore CS0618
    ...
    )
    {
        ...
        {
            throw new ValidationException("agento11y conversation rating validation failed: conversationId is required");
        }

        if (normalizedConversationId.Length > MaxRatingConversationIdLen)
        {
            throw new ValidationException("agento11y conversation rating validation failed: conversationId is too long");
        }
        ...
        {
            throw new RatingTransportException("agento11y conversation rating transport failed", ex);
        }
        using (response)
        {
            ...
            if (response.StatusCode == HttpStatusCode.BadRequest)
            {
                throw new ValidationException(
                    $"agento11y conversation rating validation failed: {RatingErrorText(responseBody, (int)response.StatusCode)}"
                );
            }
            if (response.StatusCode == HttpStatusCode.Conflict)
            {
                throw new RatingConflictException(
                    $"agento11y conversation rating conflict: {RatingErrorText(responseBody, (int)response.StatusCode)}"
                );
            }
            if (!response.IsSuccessStatusCode)
            {
                throw new RatingTransportException(
                    $"agento11y conversation rating transport failed: status {(int)response.StatusCode}: {RatingErrorText(responseBody, (int)response.StatusCode)}"
                );
            }
            if (string.IsNullOrWhiteSpace(responseBody))
            {
                throw new RatingTransportException("agento11y conversation rating transport failed: empty response payload");
            }
    ...
    public async Task ShutdownAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            _log($"agento11y generation export flush on shutdown failed: {ex}");
        }
        ...
        {
            _log($"agento11y generation exporter shutdown failed: {ex}");
        }
    ...
    private GenerationRecorder StartGenerationInternal(GenerationStart start, GenerationMode defaultMode)
    {
        ...
        {
            seed.UserId = Agento11yContext.UserIdFromContext() ?? string.Empty;
        }
        if (string.IsNullOrWhiteSpace(seed.UserId))
        {
        ...
        recorder.SetContextScope(Agento11yContext.PushContentCaptureMode(ccMode));
        return recorder;
    }
    ...
    internal void PersistGeneration(Generation generation)
    {
        ...
        {
            throw new ValidationException($"agento11y: generation validation failed: {ex.Message}");
        }
        ...
        lock (_stateLock)
        {
            if (_shutdown)
            {
                throw new ClientShutdownException("agento11y: client is shutting down");
            }
        }
        ...
        lock (_pendingLock)
        {
            ...
            {
                throw new QueueFullException("agento11y: generation queue is full");
            }
    ...
    private void TriggerBackgroundFlush()
    {
        lock (_flushBackgroundLock)
        {
            ...
            _backgroundFlushTask = Task.Run(async () =>
            {
                ...
                {
                    _log($"agento11y generation export failed: {ex}");
                }
            });
        }
    }
        ...
        try
        {
            while (true)
            {
                ...
                foreach (var result in response.Results)
                {
                    if (!result.Accepted)
                    {
                        _log($"agento11y generation rejected id={result.GenerationId} error={result.Error}");
                    }
                }
            }
        }
    ...
    private static SubmitConversationRatingRequest NormalizeConversationRatingRequest(SubmitConversationRatingRequest? request)
    {
        ...
        {
            throw new ValidationException("agento11y conversation rating validation failed: ratingId is required");
        }
        if (normalized.RatingId.Length > MaxRatingIdLen)
        {
            throw new ValidationException("agento11y conversation rating validation failed: ratingId is too long");
        }
        ...
        {
            throw new ValidationException("agento11y conversation rating validation failed: comment is too long");
        }
        if (normalized.GenerationId.Length > MaxRatingGenerationIdLen)
        {
            throw new ValidationException("agento11y conversation rating validation failed: generationId is too long");
        }
        if (normalized.RaterId.Length > MaxRatingActorIdLen)
        {
            throw new ValidationException("agento11y conversation rating validation failed: raterId is too long");
        }
        if (normalized.Source.Length > MaxRatingSourceLen)
        {
            throw new ValidationException("agento11y conversation rating validation failed: source is too long");
        }
        ...
        if (normalized.Metadata.Count > 0)
        {
            ...
            {
                throw new ValidationException($"agento11y conversation rating validation failed: metadata must be valid JSON ({ex.Message})");
            }

            if (metadataBytes.Length > MaxRatingMetadataBytes)
            {
                throw new ValidationException("agento11y conversation rating validation failed: metadata is too large");
            }
        }
    ...
    private static string BuildRatingBaseUrl(string apiEndpoint, bool insecure)
    {
        ...
        {
            throw new RatingTransportException("agento11y conversation rating transport failed: api endpoint is required");
        }
            ...
            || trimmedEndpoint.StartsWith("https://", StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                throw new RatingTransportException(
                    "agento11y conversation rating transport failed: api endpoint host is required"
                );
            }
    ...
    private static SubmitConversationRatingResponse ParseSubmitConversationRatingResponse(string payload)
    {
        try
        {
            ...
            {
                throw new RatingTransportException("agento11y conversation rating transport failed: invalid response payload");
            }
        ...
        {
            throw new RatingTransportException("agento11y conversation rating transport failed: invalid JSON response", ex);
        }
    }
    ...
    private static ConversationRating ParseConversationRating(JsonElement element)
    {
        ...
        {
            throw new RatingTransportException("agento11y conversation rating transport failed: invalid rating payload");
        }
        ...
        if (TryGetProperty(element, "metadata", out var metadataElement))
        {
            rating.Metadata = metadataElement.ValueKind switch
            {
                ...
                _ => throw new RatingTransportException("agento11y conversation rating transport failed: invalid rating payload"),
            };
        }
    ...
    private static ConversationRatingSummary ParseConversationRatingSummary(JsonElement element)
    {
        if (element.ValueKind != JsonValueKind.Object)
        {
            throw new RatingTransportException("agento11y conversation rating transport failed: invalid rating summary payload");
        }
    ...
    private static DateTimeOffset ParseTimestamp(string raw)
    {
        ...
        {
            throw new RatingTransportException("agento11y conversation rating transport failed: invalid timestamp in response payload");
        }
    ...
    private static Dictionary<string, object?> ParseMetadataObject(JsonElement value)
    {
        ...
        {
            throw new RatingTransportException("agento11y conversation rating transport failed: invalid rating payload", ex);
        }
    }
    ...
    internal static void ApplyGenerationSpanAttributes(Activity activity, Generation generation)
    {
        ...
        if (generation.Usage.CacheWrite1hInputTokens != 0)
        {
            activity.SetTag(SpanAttrCacheWrite1hTokens, generation.Usage.CacheWrite1hInputTokens);
        }
    ...
    )
    {
        ...
        RecordTokenUsage(generation, MetricTokenTypeCacheWrite1h, generation.Usage.CacheWrite1hInputTokens);
    ...
    // Recomputed on every call: Agento11yClientConfig.Tags exposes the live
        ...
        Action<string>? logger = null)
    {
        ...
        {
            logger?.Invoke($"agento11y: content capture resolver threw, falling back to MetadataOnly: {ex.Message}");
            return ContentCaptureMode.MetadataOnly;
        }

        if (!Enum.IsDefined(typeof(ContentCaptureMode), mode))
        {
            logger?.Invoke($"agento11y: content capture resolver returned undefined mode {(int)mode}, falling back to MetadataOnly");
            return ContentCaptureMode.MetadataOnly;
        }
...
public sealed class GenerationRecorder
{
    ...
    private readonly Agento11yClient? _client;
    ...
    public void End()
    {
        ...
        try
        {
            ...
            if (_client!._config.GenerationSanitizer != null && effectiveContentCaptureMode != ContentCaptureMode.MetadataOnly)
            {
                ...
                catch (Exception ex)
                {
                    _client._config.Logger?.Invoke(
                        $"agento11y: generation sanitization failed, falling back to metadata_only: {ex.Message}"
                    );
            ...
            {
                generation.Metadata[Agento11yClient.MetadataKeyContentCaptureMode] = modeValue;
            }

            if (effectiveContentCaptureMode == ContentCaptureMode.MetadataOnly)
            {
                var stripErrorCategory = Agento11yClient.ErrorCategoryFromException(callError, false);
                Agento11yClient.StripContent(generation, stripErrorCategory);
            }
        ...
        if (_activity != null)
        {
            ...
            _activity.DisplayName = Agento11yClient.GenerationSpanName(generation.OperationName, generation.Model.Name);
            ...
            if (effectiveContentCaptureMode == ContentCaptureMode.FullWithMetadataSpans)
            {
                ...
                try
                {
                    Agento11yClient.ApplyGenerationSpanAttributes(_activity, generation);
                }
                finally
                {
            ...
            {
                _activity.SetTag(Agento11yClient.SpanAttrConversationTitle, generation.ConversationTitle);
            }
            ...
            if (callError != null)
            {
                ...
                Agento11yClient.RecordException(_activity, spanError, redactErrors);
            }

            if (mappingError != null)
            {
                Agento11yClient.RecordException(_activity, mappingError, redactErrors);
            }
        }
        ...
        {
            localError = new EnqueueException($"agento11y: generation enqueue failed: {ex.Message}", ex);
        }
        ...
        if (callError != null)
        {
            ...
            errorCategory = Agento11yClient.ErrorCategoryFromException(callError, true);
        }
        ...
        if (_activity != null)
        {
            if (localError != null)
            {
                Agento11yClient.RecordException(_activity, localError, redactErrors);
            }

            if (errorType.Length > 0)
            {
                _activity.SetTag(Agento11yClient.SpanAttrErrorType, errorType);
                _activity.SetTag(Agento11yClient.SpanAttrErrorCategory, errorCategory);
    ...
    )
    {
        ...
        generation.OperationName = FirstNonEmpty(
            ...
            _seed.OperationName,
            Agento11yClient.DefaultOperationNameForMode(generation.Mode.Value)
        );
        ...
        generation.ConversationTitle = FirstNonEmpty(
            ...
            MetadataString(generation.Metadata, Agento11yClient.SpanAttrConversationTitle)
        );
        generation.ConversationTitle = NormalizeResolvedString(generation.ConversationTitle);
        if (!string.IsNullOrWhiteSpace(generation.ConversationTitle))
        {
            generation.Metadata[Agento11yClient.SpanAttrConversationTitle] = generation.ConversationTitle;
        }

        generation.UserId = FirstNonEmpty(
            generation.UserId,
            MetadataString(generation.Metadata, Agento11yClient.MetadataUserIdKey),
            MetadataString(generation.Metadata, Agento11yClient.MetadataLegacyUserIdKey)
        );
        generation.UserId = NormalizeResolvedString(generation.UserId);
        if (!string.IsNullOrWhiteSpace(generation.UserId))
        {
            generation.Metadata[Agento11yClient.MetadataUserIdKey] = generation.UserId;
        }
        ...
        generation.Metadata[Agento11yClient.SpanAttrSdkName] = Agento11yClient.SdkName;
    ...
    private static void SyncCanonicalMetadataMirrors(Generation generation)
    {
        ...
        {
            generation.Metadata.Remove(Agento11yClient.SpanAttrConversationTitle);
        }
...
public sealed class EmbeddingRecorder
{
    ...
    public void End()
    {
        ...
        {
            localError = new ValidationException($"agento11y: embedding validation failed: {ex.Message}");
        }
        ...
        if (_activity != null)
        {
            _activity.DisplayName = Agento11yClient.EmbeddingSpanName(_seed.Model.Name);
            Agento11yClient.ApplyEmbeddingStartSpanAttributes(_activity, _seed);
            Agento11yClient.ApplyEmbeddingEndSpanAttributes(_activity, result, _client.EmbeddingCapture, _contentCaptureMode);

            if (callError != null)
            {
                Agento11yClient.RecordException(_activity, callError, redactSpanErrors);
            }
            if (localError != null)
            {
                Agento11yClient.RecordException(_activity, localError, redactSpanErrors);
            }
...
public sealed class ToolExecutionRecorder
{
    ...
    public void End()
    {
        ...
        if (_activity != null)
        {
            _activity.DisplayName = Agento11yClient.ToolSpanName(_seed.ToolName);
            Agento11yClient.ApplyToolSpanAttributes(_activity, _seed);
            ...
            if (_includeContent)
            {
                try
                {
                    ...
                    {
                        _activity.SetTag(Agento11yClient.SpanAttrToolCallArguments, arguments);
                    }

                    var resultJson = InternalUtils.SerializeJson(result.Result);
                    if (!string.IsNullOrWhiteSpace(resultJson))
                    {
                        _activity.SetTag(Agento11yClient.SpanAttrToolCallResult, resultJson);
                    }
                }
                catch (Exception ex)
                {
            ...
            if (finalError != null)
            {
                Agento11yClient.RecordException(_activity, finalError, _redactErrors);
                var errorCategory = Agento11yClient.ErrorCategoryFromException(finalError, true);
                _activity.SetTag(Agento11yClient.SpanAttrErrorType, "tool_execution_error");
                _activity.SetTag(Agento11yClient.SpanAttrErrorCategory, errorCategory);
