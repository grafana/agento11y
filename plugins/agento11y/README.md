...
The binary carries agent skills: markdown workflows a coding agent reads to do a job. They ship inside the binary, so there is nothing to fetch and no second CLI to install. A skill is updated by upgrading `agento11y`.
...
`agento11y doctor` and `agento11y login` both name that command when they finish, so you do not have to remember it.
...
`agento11y help` (or `--help`, or `-h`) prints the full command list on stdout and exits 0. Misuse prints a one-line usage form on stderr and exits 2: an unknown subcommand, or a command given the wrong number of arguments.
