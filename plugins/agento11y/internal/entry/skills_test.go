...
func TestRun_SkillsList(t *testing.T) {
	...
	// Each bundled skill gets one line carrying its name and a description, so
	// a reader can pick without printing the file.
...
// TestRun_HelpIsARealCommand pins the change from the old behaviour, where
// `agento11y help` fell through to the arity guard and printed the usage line
// to stderr with exit 2. Help now answers on stdout and exits 0.
...
//
// The subcommand list is hand-written, because run dispatches with a flat
// if-chain over args[0] rather than a table a test can read: adding a
// subcommand to run and forgetting this list is not caught. The launcher names
// are read from the launchers map, so those are.
