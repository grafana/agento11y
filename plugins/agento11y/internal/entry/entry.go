...
func run(args []string, stdin io.Reader, stdout, stderr io.Writer) {
	...
	// `agento11y help` answers on stdout and exits 0. Before this branch
	// existed, `help` and `--help` fell through to the arity guard below,
	// printed the one-line usage form on stderr, and exited 2.
	...
	// `agento11y skills` prints the agent skills bundled into this binary. It
	// sits next to `doctor`, above launcher and hook dispatch, so an agent
	// named `skills` could never shadow it.
