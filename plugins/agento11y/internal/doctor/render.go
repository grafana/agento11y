...
func renderHuman(w io.Writer, r *Report, color bool) {
	...
	// Summary, then the hint that hands the fix to a coding agent.
	writeSummary(&b, p, r)
...
// and that reader has wired nothing and needs the paste block most.
...
// noise.
//
// Both lines start at column 0: the report reads an indented line as a
// key/value row.
func writeSetupHint(b *reportBody, p palette, r *Report) {
	...
	// Only the introduction is faint. The line below it is meant to be copied,
