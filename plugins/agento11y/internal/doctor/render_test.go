...
func TestDescribeLocalRules(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing", config: ConfigSection{GuardsFile: path}, want: "missing (" + path + ")"},
		{name: "enforcing", config: ConfigSection{GuardsFile: path, GuardsFileExists: true, GuardsFileRules: 2}, want: "2 enforcing (" + path + ")"},
		{name: "zero still reports the count", config: ConfigSection{GuardsFile: path, GuardsFileExists: true}, want: "0 enforcing (" + path + ")"},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			if got := describeLocalRules(p, tc.config); got != tc.want {
				t.Fatalf("describeLocalRules = %q, want %q", got, tc.want)
			}
		})
	}
}
