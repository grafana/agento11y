...
func goldenHealthyReport() *Report {
	...
	return &Report{
		...
		Config: ConfigSection{
			...
			// The opt-in switch, fully resolved: this is the row a user reads to
			// see which strings become metric labels.
			AutoTagNames:       []string{"user", "repo"},
			AutoTags:           map[string]string{"user": "alice@example.com", "repo": "grafana/agento11y"},
			AutoTagsKey:        "AGENTO11Y_AUTO_CODING_AGENT_TAGS",
			AutoTagsSource:     sourceConfig,
			AutoTagNamesKey:    "AGENTO11Y_AUTO_CODING_AGENT_TAGS_NAMES",
			AutoTagNamesSource: sourceConfig,
...
func goldenBrokenReport() *Report {
	...
	return &Report{
		...
		Config: ConfigSection{
			...
			// The switch is on, but one name is misspelled, one is overridden by an
			// explicit tag, and one found no git checkout. Only `user` reaches the
			// client, so the row shows one value for three enabled names.
			Tags:               map[string]string{"repo": "monorepo"},
			TagsKey:            "AGENTO11Y_TAGS",
			TagsSource:         sourceConfig,
			AutoTagNames:       []string{"user", "repo", "branch"},
			AutoTags:           map[string]string{"user": "alex"},
			AutoTagUnresolved:  []string{"branch"},
			AutoTagShadowed:    []string{"repo"},
			AutoTagUnknown:     []string{"team"},
			AutoTagsKey:        "AGENTO11Y_AUTO_CODING_AGENT_TAGS",
			AutoTagsSource:     sourceConfig,
			AutoTagNamesKey:    "AGENTO11Y_AUTO_CODING_AGENT_TAGS_NAMES",
			AutoTagNamesSource: sourceConfig,
