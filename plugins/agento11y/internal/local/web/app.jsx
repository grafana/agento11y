      ...
      <div style={{ position: 'relative' }}>
        ...
        >
          ...
          {hover !== null && (
            ...
            >
              <span style={{ color: 'var(--fg3)' }}>{data[hover].t}</span> · {data[hover].c}{' '}
          ...
          {hover !== null && visibleTotal(data[hover]) > 0 && (
            ...
            >
              <div style={{ color: 'var(--fg3)', marginBottom: 4 }}>
                {data[hover].t} · {formatTokens(visibleTotal(data[hover]))} tok
              </div>
              {visible
...
function Select({ value, options, onChange, title, trigger, menu, id, disabled, icon, prefix }) {
...
function WorkspaceFacet({ workspaces, selected, onSelect, totalCount, totalCost, now, rangeLabel }) {
  const [open, setOpen] = useState(false);
          ...
          <div id="workspace-facet-listbox" role="listbox" aria-label="Workspaces">
            ...
            <div ref={listRef} style={{ maxHeight: 296, overflowY: 'auto' }}>
              {shown.map((w, i) => {
                ...
                const share = shareAvailable ? w.cost / totalCost : null;
...
function ConvRow({ c, now, onOpen, prices, grouped = false, hideWorkspace = false }) {
...
function SessionGroupHeader({ groupBy, label, open, onToggle, count, cost, tokens, last, share, now }) {
...
function WorkspaceContextStrip({ path, count, cost, tokens, last, share, now, onClear }) {
...
function ConversationsView({
  ...
  const tokenModels = useMemo(() => Array.from(new Set(points.map((p) => p.model).filter(Boolean))).sort(), [points]);
      ...
      {searchActive ? (
        ...
        <React.Fragment>
          ...
          >
            ...
            {!error && (!loading || selectedWorkspaceRangeAggregate?.count > 0) && selectedWorkspaceRangeAggregate && (
            ...
            {!error &&
              ...
              activeWorkspace == null &&
              rangeFiltered.length === 0 &&
              (storeCount > 0 || conversations.length > 0) && (
                <div
                  style={{
                    padding: '16px 18px',
                    color: 'var(--fg2)',
            ...
            {!error &&
              filtered.length === 0 &&
              rangeFiltered.length > 0 &&
              (activeWorkspace == null || selectedWorkspaceRangeAggregate?.count > 0) && (
                <div
                  style={{
                    padding: '16px 18px',
                    color: 'var(--fg2)',
            ...
            {groupBy === 'none'
              ...
              : groupBy === 'workspace' && activeWorkspace != null
                ...
                : grouped.map((group, index) => {
                    const open = groupOpen.has(group.key) ? groupOpen.get(group.key) : index < OPEN_GROUPS;
                    return (
                      <React.Fragment key={`${groupBy}:${group.key}`}>
                        <SessionGroupHeader
                          groupBy={groupBy}
                          label={group.key}
                          open={open}
...
function WorthALook({ steps, turns, metrics, onJump }) {
  ...
  const pickStep = (value) =>
    (steps || []).reduce((current, gen) => (!current || value(gen) > value(current) ? gen : current), null);
...
function SettingsConnectFlow({ savedStackURL, configPath, capture, onConnect, onManual }) {
...
function SettingsView({ history, config, configError, activeSettingsTab, onSelectTab, onConfig }) {
    ...
    oneClickWrite(
      {
        ...
        localForward: true,
        ...(captureForwardMode(saved.capture) === mode ? {} : { capture: mode }),
      },
  ...
  const commitMode = (mode, forceLocalOff = false) => {
    const patch = forwardLocalPatch(saved, mode) || (forceLocalOff && mode === 'off' ? { localForward: false } : null);
    if (!patch) return;
    oneClickWrite(
      patch,
      mode === 'off'
