...
function makeConfig(
  overrides?: Partial<Agento11yDshConfig>,
): Agento11yDshConfig {
  return {
...
describe("createAgento11yClient", () => {
      ...
      generationExport: {
        ...
        headers: {
          "User-Agent": expect.stringMatching(
            /^agento11y-plugin-dsh\/.+ agento11y-sdk-js\/0\.0\.0-test$/,
          ),
        },
      },
  ...
  it("sets the plugin User-Agent on the generation export", () => {
    ...
    expect(ua.startsWith("agento11y-plugin-dsh/")).toBe(true);
  ...
  it("configures no hooks block, because this plugin evaluates no guards", () => {
    createAgento11yClient(makeConfig());
    const [arg] = Agento11yClientMock.mock.calls[0]!;
    expect("hooks" in arg).toBe(false);
  });
