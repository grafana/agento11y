...
// Redirect homedir so fallback tests never read a live user config.
...
describe("agento11yConfigEnvPath", () => {
  ...
  beforeEach(() => {
    clearAgento11yEnv();
    fakeHome = mkdtempSync(join(tmpdir(), "agento11y-dsh-home-"));
  ...
  it("honors an absolute XDG_CONFIG_HOME", () => {
    // Existing config files change path selection, so use a fresh root.
...
describe("loadAgento11yDotenv", () => {
  ...
  beforeEach(() => {
    clearAgento11yEnv();
    dir = mkdtempSync(join(tmpdir(), "agento11y-dsh-dotenv-"));
  });
  ...
  it("warns and returns an empty map on non-ENOENT read failures", () => {
    // A directory path is a read error, not a missing-file result.
    const warn = loggerMock.warn;
    warn.mockClear();
...
describe("applyAgento11yDotenv", () => {
  ...
  beforeEach(() => {
    clearAgento11yEnv();
    dir = mkdtempSync(join(tmpdir(), "agento11y-dsh-dotenv-apply-"));
  ...
  it("materializes both local-mode variables under both spellings", () => {
    // LOCAL and BIN need alias precedence even though the SDK does not read them.
