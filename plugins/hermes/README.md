...
This is an inactive snapshot import from commit `4f946a2f1ecfda2e8ebccd35faa13d81db64b0b2` of `alexander-akhmetov/grafana-agento11y-hermes`. The package version remains `0.10.0`. Monorepo changes are unreleased, with no release workflow or `.github/sdk-releases.json` registration. See [CHANGELOG.md](CHANGELOG.md) for the historical release and separate migration notes.

Current source: https://github.com/grafana/agento11y/tree/main/plugins/hermes

## Install the snapshot

The shared launcher does not support Hermes yet. `agento11y login`, its shared `config.env`, and its local mode do not configure this plugin.

Install into the Python environment that runs Hermes, not an unrelated system Python. Python 3.11 or newer is required. Hermes 0.16.0 is the supported floor; the source snapshot was tested against Hermes 0.19.0.

From the monorepo root, with Hermes's Python environment active:

```sh
python -m pip install ./plugins/hermes
```

The package keeps the name `grafana-agento11y-hermes`. Its `hermes_agent.plugins` entry point remains `agento11y = grafana_agento11y_hermes`. Installing the published PyPI `0.10.0` does not install these unreleased privacy changes.
...
## Privacy in the monorepo migration

The migration uses Python SDK 0.17.x and aligns privacy with sibling plugins instead of preserving the snapshot's full-content default.
