 ...
 * id does not match, falls back to the only rewritten tool call, or the only
 * rewritten call with the same tool name. Returns `undefined` on any mismatch
 * or parse failure so the caller falls through to the original tool input.
 *
